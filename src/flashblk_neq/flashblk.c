...
|  Revisions:
|
|     1/2/07   SPJ   Bank size and locations were #defed based on MCU type. Made
|                    them 'extern'.
|
|--------------------------------------------------------------------------*/
...
PRIVATE T_BankTag tagFromPage( T_FlashAddr pageAddr ) { return *(T_BankTag CONST *)pageAddr; }
...
#endif
#if 1
PUBLIC BIT handleEquals_flashBlk(T_FlashAddr p, T_FlashBlk_Handle hdl )
{
   return FlashBlk_GetHdl(p) == hdl;
}
#endif
/*-----------------------------------------------------------------------------------
|
...
//   return (FlashBlk_GetHdl(s) != 0x00) && (FlashBlk_GetHdl(s) != 0xFF);
   return !handleEquals_flashBlk(s, 0x00) && !handleEquals_flashBlk(s, 0xFF);
}
...
|
|  Returns start address of the currently active bank, 0 if the bank has not been
|  initialised.
|
--------------------------------------------------------------------------------------*/
...
PRIVATE T_FlashAddr firstEmptySlot(void)
{
   ...
   for( p = currentBank(); !handleEquals_flashBlk(p, _ErasedHandle); p += _SlotBytes )
   {
      if( p >= bankEnd() )
         { return 0; }
   }
...
PUBLIC T_FlashAddr getVarAddr_flashBlk( T_FlashBlk_Handle hdl )
{
   ...
   for( p = currentBank(); !handleEquals_flashBlk(p, hdl); p += _SlotBytes )
   {
      if( p >= bankEnd() )
         { return 0; }
   }
...
PUBLIC U8 FlashBlk_ReadOfs( U8 *out, T_FlashBlk_Handle hdl, U8 maxRead, U8 ofs )
{
   ...
   */
   for( bytesRead = 0, readIdx = 0;; )
   {
      ...
      else if( !handleEquals_flashBlk(p, hdl) )    // next slot not a match to 'hdl'?
      {
         break;
      }
   }
